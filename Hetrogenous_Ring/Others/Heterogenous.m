...
E1=10;                    % Initial Energy of Sensor Node in Joule
Power = 30;
comm_radius=80;           % Communication range of nodes is 80 meter
rmax=10;                   % Maximum number of rounds
a1=1;                     % alpha
b1=2;                     % beta
Packet_Loss = 0;          % 
s = 0;                    % For the sink movement.
     ...
     %%% Reducing energy due to control packet
        ...
        for i = 1:n
            ...
            for j = 1:size(Anch_Path,2)-1
                ...
                if (i == 1)
                    if (distance >=do)
                        S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance * distance * distance)); 
                    else
                        S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance)); 
                    end
                end
                if (distance >=do)
                    S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance * distance * distance)); 
                    S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ERX+EDA)*(CS) + Emp * CS *(distance * distance * distance * distance)); 
                else
                    S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance)); 
                    S(Anch_Path(j)).E = S(Anch_Path(j)).E - ((ERX+EDA)*(CS) + Emp * CS *(distance * distance)); 
                end
            end
            if (distance >=do)
                S(Anch_Path(j + 1)).E = S(Anch_Path(j + 1)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance * distance * distance)); 
            else
                S(Anch_Path(j + 1)).E = S(Anch_Path(j + 1)).E - ((ETX+EDA)*(CS) + Emp * CS *(distance * distance)); 
            end
        end
        for i = 1: size(Anch, 2)
            if (S(Anch(i)).E <= 1)
